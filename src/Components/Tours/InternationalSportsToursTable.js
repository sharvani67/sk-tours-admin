...
const INTLSportsTours = () => {
  ...
  const fetchTours = async () => {
    try {
      ...
      const filteredTours = result.filter(tour => 
        tour.tour_type && tour.tour_type.toLowerCase() === "sports" &&  tour.is_international === 1
      );
  ...
  const handleViewTour = (tourId) => {
    navigate(`/intl-sports-tour-details/${tourId}`);
  };

  // Handle edit tour
  const handleEditTour = (tourId) => {
    // Navigate to edit page with tour ID
    navigate(`/intl-edit-sports-tour/${tourId}`);
  };
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <div className="d-flex gap-2">
            <button
              ...
              onClick={() => navigate('/intl-add-sports-tour')}
            >
              + Add Tour
            </button>
          </div>
...
export default INTLSportsTours;