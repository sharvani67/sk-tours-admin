...
// Visa Form - Start with empty array instead of predefined types
  ...
  const loadDropdownsAndTourCode = async () => {
    try {
      ...
      if (isEditMode) {
        ...
        const tourCodeRes = await fetch(`${baseurl}/api/tours/next-tour-code?tour_type=sports&is_international=${isInternational}`);
        if (tourCodeRes.ok) {
...
const visaData = {
  ...
  visa_forms: uploadedVisaForms.map(form => ({
    visa_type: form.type, // Use visa_type for backend
    type: form.type, // Keep type for frontend consistency
