...
const INTLSportsAddTour = () => {
    ...
    const loadDropdownsAndTourCode = async () => {
      try {
        ...
        if (isEditMode) {
          ...
          const tourCodeRes = await fetch(`${baseurl}/api/tours/next-tour-code?tour_type=sports&is_international=1`);
          if (tourCodeRes.ok) {
  ...
  const createTour = async () => {
    ...
    try {
      ...
      setTimeout(() => navigate('/intl-sports-tours'), 1500);
    } catch (err) {
...
export default INTLSportsAddTour;