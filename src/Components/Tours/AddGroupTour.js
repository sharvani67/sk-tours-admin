...
// Add editing states for all tabs
...
// Add this useEffect to set initial prefilled content for Group Tour
useEffect(() => {
...
const [transportItem, setTransportItem] = useState({
  ...
  to_time: '',
  via: '',
  sort_order: transports.length + 1  // Make sure sort_order is properly set
});
    ...
    try {
      ...
      if (data.success) {
       ...
       if (data.hotels && Array.isArray(data.hotels)) {
        ...
        // Set transport
   // In loadTourData() function, transport section should already be correct:
...
useEffect(() => {
 ...
 const handleRemoveItinerary = (idx) => {
  ...
  const autoAddBeforeNext = () => {
    switch (activeTab) {
       case 'itineraries':
      // Only auto-add if not in edit mode
      ...
      break;
       ...
       case 'departures':
      // Only auto-add if not in edit mode
      if (groupDepartureForm.start_date && groupDepartureForm.end_date && editingDepartureIndex === -1) {
        handleAddDeparture();
      }
      break;

      //   case 'costs':
      // // For costs tab, check if there's a selected departure with filled cost fields
      // if (selectedDepartureForCost !== null && 
      //     (groupDepartureForm.three_star_twin || 
      //      groupDepartureForm.four_star_twin || 
      //      groupDepartureForm.five_star_twin)) {
      //   handleUpdateDepartureCost(selectedDepartureForCost);
      // }
      // break;
...
const handleSaveClick = () => {
  ...
  if (isLastTab) {
    ...
    const message = isEditMode 
      ...
      : 'Are you sure you want to save this group tour and all its details?';
