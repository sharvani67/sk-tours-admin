...
const AddSeniorTour = () => {
  ...
  // =======================
  // VISA STATE FOR INTERNATIONAL SENIOR TOURS
  // =======================
  // Tourist Visa
    ...
    const loadDropdownsAndTourCode = async () => {
      try {
        ...
        if (isEditMode) {
          ...
          const tourCodeRes = await fetch(`${baseurl}/api/tours/next-tour-code?tour_type=seniorcitizen&is_international=1`);
          if (tourCodeRes.ok) {
  ...
  const handleCancel = () => {
    navigate('/intl-senior-citizen-tours');
  };
  ...
  const updateTour = async () => {
    ...
    try {
      ...
      setTimeout(() => navigate('/intl-senior-citizen-tours'), 1500);
    } catch (err) {
  ...
  const handleSaveClick = () => {
    ...
    if (isLastTab) {
      const message = isEditMode 
        ? 'Are you sure you want to update this international senior citizen tour with all changes?'
        : 'Are you sure you want to save this international senior citizen tour and all its details?';
  ...
  return (
    <Navbar>
      <Container>
        <h2 className="mb-4">{isEditMode ? 'Edit International Senior Citizen Tour' : 'Add International Senior Citizen Tour'}</h2>
