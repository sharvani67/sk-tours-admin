...
const INTLFestivalAddTour = () => {
  ...
  useEffect(() => {
    if (!isEditMode) {
      ...
      setTouristVisaRemarksOption2("Premium visa service with express processing. Doorstep document pickup. Guaranteed appointment within 2 days. 24/7 visa assistance available. Visa approval rate 99%.");
    ...
    const loadDropdownsAndTourCode = async () => {
      try {
        ...
        if (isEditMode) {
          ...
          const tourCodeRes = await fetch(`${baseurl}/api/tours/next-tour-code?tour_type=festival&is_international=1`);
          if (tourCodeRes.ok) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          tour_type: basic.tour_type || 'festival',
          title: basic.title || '',
  ...
  const createTour = async () => {
    ...
    try {
      ...
      const completeFormData = {
        ...
        tour_type: formData.tour_type || 'festival',
  ...
  const handleSaveClick = () => {
    if (isLastTab) {
      const confirmMessage = isEditMode 
        ? 'Are you sure you want to update this festival tour with all changes?' 
        : 'Are you sure you want to save this festival tour with all data?';
  ...
  return (
    <Navbar>
      <Container>
        <h2 className="mb-4">{isEditMode ? 'Edit Festival Tour' : 'Add Festival Tour'}</h2>
