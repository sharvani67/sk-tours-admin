...
const AddStudentTour = () => {
  ...
  // ========================
  // STATE FOR OPTION TABS (Option 1 / Option 2) - STUDENT TOUR
  // ========================
  ...
  // =======================
  // VISA STATE FOR INTERNATIONAL STUDENT TOURS
  // =======================
  // Tourist Visa
  ...
  const removeTouristVisa = (idx) => {
    ...
    if (confirmDelete) {
      ...
      if (editingType === 'touristVisa' && editIndex === idx) {
        resetEditing();
      }
    }
  };

  // Transit Visa Functions
  ...
  const removeTransitVisa = (idx) => {
    ...
    if (confirmDelete) {
      ...
      if (editingType === 'transitVisa' && editIndex === idx) {
        resetEditing();
      }
    }
  };

  // Business Visa Functions
  ...
  const removeBusinessVisa = (idx) => {
    ...
    if (confirmDelete) {
      ...
      if (editingType === 'businessVisa' && editIndex === idx) {
        resetEditing();
      }
    }
  };
  ...
  const removePhoto = (idx) => {
    ...
    if (confirmDelete) {
      ...
      if (editingType === 'photo' && editIndex === idx) {
        resetEditing();
      }
    }
  };

  // Free Flow Photo Entry Functions
  ...
  useEffect(() => {
    if (!isEditMode) {
      ...
      setTouristVisaRemarksOption2("Express visa processing available for additional fee. Visa on arrival not available for student visas. Sponsored visa options available. Work permit restrictions apply. Contact visa officer for specific requirements.");
    ...
    const loadDropdownsAndTourCode = async () => {
      try {
        ...
        if (isEditMode) {
          ...
          const tourCodeRes = await fetch(`${baseurl}/api/tours/next-tour-code?tour_type=student&is_international=1`);
          if (tourCodeRes.ok) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.visa_details && data.visa_details.length > 0) {
          const firstVisa = data.visa_details[0];
          touristVisaRemarksValue = firstVisa.remarks || '';
          touristVisaRemarksOpt1 = firstVisa.remarks_option1 || '';
          touristVisaRemarksOpt2 = firstVisa.remarks_option2 || '';
          touristVisaRemarksActive = firstVisa.remarks_active || 'option1';
        }
  ...
  const autoAddBeforeNext = () => {
    switch (activeTab) {
      case 'itineraries':
        ...
        break;
      case 'departures':
        if (studentDepartureForm.start_date && studentDepartureForm.end_date && editingDepartureIndex === -1) {
          handleAddDeparture();
        }
        break;
      case 'optionalTours':
  ...
  const updateTour = async () => {
    ...
    try {
      ...
      const tourUpdateData = {
        ...
        is_international: Number(formData.is_international) || 1,
  ...
  const handleSaveClick = () => {
    ...
    if (isLastTab) {
      const message = isEditMode 
        ? 'Are you sure you want to update this international student tour with all changes?'
        : 'Are you sure you want to save this international student tour and all its details?';
  ...
  return (
    <Navbar>
      <Container>
        <h2 className="mb-4">{isEditMode ? 'Edit International Student Tour' : 'Add International Student Tour'}</h2>
              ...
              <Tab eventKey="visa" title="Visa">
                ...
                >
                  ...
                  <Tab eventKey="form" title="Visa Form">
                    ...
                    {/* Tourist Visa Remarks with Option Tabs */}
                    <Card className="mt-3">
                      <Card.Body>
                        <Form.Group>
