...
const resetVisaEditing = () => {
  ...
  // Also reset any visa form fields that might be in edit mode
...
// Add this state for editing context
...
// Add this function for handling file uploads with edit support
const handleVisaFormFileChangeWithEdit = async (index, action, file) => {
  ...
  if (isEditMode && id) {
    ...
    if (uploadedFileName) {
      if (editingVisaFormIndex !== null && editingVisaFormIndex === index) {
        // Update edit data with filename
        setVisaFormEditData(prev => ({
          ...prev,
          [action === 'action1' ? 'action1_file' : 'action2_file']: uploadedFileName
        }));
      } else {
        // Update main list with filename
        const updatedWithFilename = [...visaFormItems];
...
// Update your existing edit functions for visa items (currently they delete the item when editing)
const editTouristVisa = (idx) => {
  const item = touristVisaItems[idx];
  setTouristVisaForm({ description: item.description });
  // Set editing context instead of removing from list
  setEditingItem(item);
...
// Update the add functions to handle editing
  ...
  // Dynamic "+ Add ..." button for bottom bar
   const getAddConfigForTab = (tabKey) => {
    switch (tabKey) {
      ...
      case 'cancellation':
        return { 
          label: editingCancellationIndex !== -1 ? '✓ Update Cancellation Policy' : '+ Add Cancellaton Policy', 
               ...
               <Tab eventKey="visa" title="Visa">
                              ...
                              >
                                ...
                                <Tab eventKey="tourist" title="Tourist Visa">
                                  <Form.Group className="mb-3">
                                    <Form.Label>Toursit Visa Entry</Form.Label>
                                    <Form.Control
                                      as="textarea"
                                      rows={4}
                                      name="description"
