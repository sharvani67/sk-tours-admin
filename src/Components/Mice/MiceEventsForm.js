...
const EventsForm = ({ 
      ...
      if (file.type.startsWith('image/') && file.size <= 10 * 1024 * 1024) {
        ...
        reader.onloadend = () => {
          ...
          if (newPreviews.length === validFiles.length) {
            setEventsForm({
              ...eventsForm,
              images: [...eventsForm.images, ...validFiles],
              imagePreviews: [...eventsForm.imagePreviews, ...newPreviews]
            });
            setError('');
          }
        };
  ...
  const removeImage = (index) => {
    const updatedImages = eventsForm.images.filter((_, i) => i !== index);
    const updatedPreviews = eventsForm.imagePreviews.filter((_, i) => i !== index);
    setEventsForm({
      ...eventsForm,
  ...
  const handleSubmit = async (e) => {
    ...
    if (eventsForm.images.length === 0) {
      setError('Please select at least one image to upload');
      return;
    }

    const formData = new FormData();
    eventsForm.images.forEach(image => {
      formData.append('images', image);
    });

    try {
      const response = await fetch(`${baseurl}/api/mice/events`, {
        method: 'POST',
        body: formData
      });
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label>Select Multiple Event Images *</label>
          <input
            type="file"
          ...
          <div className="image-previews-grid d-flex flex-wrap gap-2 mt-2">
            {eventsForm.imagePreviews.map((preview, index) => (
