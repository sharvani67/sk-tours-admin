...
const VenuesForm = ({ 
      ...
      if (file.type.startsWith('image/') && file.size <= 10 * 1024 * 1024) {
        ...
        reader.onloadend = () => {
          ...
          if (newPreviews.length === validFiles.length) {
            setVenuesForm({
              ...venuesForm,
              images: [...venuesForm.images, ...validFiles],
              imagePreviews: [...venuesForm.imagePreviews, ...newPreviews]
            });
            setError('');
          }
        };
  ...
  const removeImage = (index) => {
    const updatedImages = venuesForm.images.filter((_, i) => i !== index);
    const updatedPreviews = venuesForm.imagePreviews.filter((_, i) => i !== index);
    setVenuesForm({
      ...venuesForm,
  ...
  const handleSubmit = async (e) => {
    ...
    if (venuesForm.images.length === 0) {
      setError('Please select at least one image to upload');
      return;
    }

    const formData = new FormData();
    venuesForm.images.forEach(image => {
      formData.append('images', image);
    });

    try {
      const response = await fetch(`${baseurl}/api/mice/venues`, {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <>
      <h2>Upload Venue Images</h2>
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label>Select Multiple Venue Images *</label>
          <input
            type="file"
          ...
          <div className="image-previews-grid d-flex flex-wrap gap-2 mt-2">
            {venuesForm.imagePreviews.map((preview, index) => (
