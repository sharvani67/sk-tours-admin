...
const ClientsForm = ({ 
      ...
      if (file.type.startsWith('image/') && file.size <= 10 * 1024 * 1024) {
        ...
        reader.onloadend = () => {
          ...
          if (newPreviews.length === validFiles.length) {
            setClientsForm({
              ...clientsForm,
              images: [...clientsForm.images, ...validFiles],
              imagePreviews: [...clientsForm.imagePreviews, ...newPreviews]
            });
            setError('');
          }
        };
  ...
  const removeImage = (index) => {
    const updatedImages = clientsForm.images.filter((_, i) => i !== index);
    const updatedPreviews = clientsForm.imagePreviews.filter((_, i) => i !== index);
    setClientsForm({
      ...clientsForm,
  ...
  const handleSubmit = async (e) => {
    ...
    if (clientsForm.images.length === 0) {
      setError('Please select at least one image to upload');
      return;
    }

    const formData = new FormData();
    clientsForm.images.forEach(image => {
      formData.append('images', image);
    });

    try {
      const response = await fetch(`${baseurl}/api/mice/clients`, {
        method: 'POST',
        body: formData
      });
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label>Select Multiple Client Images *</label>
          <input
            type="file"
          ...
          <div className="image-previews-grid d-flex flex-wrap gap-2 mt-2">
            {clientsForm.imagePreviews.map((preview, index) => (
