...
const AddMiceDetails = () => {
  ...
  // VISA SECTION (for international only)
  ...
  const updateImage = async (imageId) => {
    ...
    try {
      ...
      const deleteResponse = await fetch(`${baseurl}/api/mice/mice-images/${imageId}`, {
        method: 'DELETE'
      });
      ...
      const uploadResponse = await fetch(`${baseurl}/api/mice/mice-images/upload/${id}`, {
        method: 'POST',
        body: formData
      });
  ...
  const deleteImage = async (imageId) => {
    ...
    try {
      ...
      const response = await fetch(`${baseurl}/api/mice/mice-images/${imageId}`, {
        method: 'DELETE'
      });
  ...
  const setCoverImage = async (imageId) => {
    try {
      ...
      const response = await fetch(`${baseurl}/api/mice/mice-images/cover/${imageId}`, {
        method: 'PUT'
      });
 ...
 const handleSaveClick = () => {
  ...
  if (isLastTab) {
    ...
    const confirmMessage = isEditMode 
      ? 'Are you sure you want to update this MICE tour with all changes?'
      : 'Are you sure you want to save this MICE tour with all data?';
      ...
      <Navbar>
        <Container>
          <Alert variant="warning" className="mt-4">
            <Alert.Heading>No Mice Selected</Alert.Heading>
            <p>Please go back and select a mice first.</p>
            <hr />
            <div className="d-flex justify-content-end">
              <Button onClick={() => navigate('/mice')} variant="primary">
                Go Back to Mice
              </Button>
            </div>
          </Alert>
    ...
    return (
      <Navbar>
        <Container className="text-center py-5">
          ...
          <p className="mt-2">Loading mice details...</p>
        </Container>
      </Navbar>
    );
  }
